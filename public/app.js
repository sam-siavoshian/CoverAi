document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add sound detector to the global variables
    ...
    async function processVoiceMessage(audioFile) {
        try {
            ...
            // Add any detected sounds to the form data
            if (detectedSounds && detectedSounds.length > 0) {
                formData.append('detected_sounds', JSON.stringify(detectedSounds));
                console.log('Adding detected sounds to request:', detectedSounds);
            }
            ...
            console.log('Sending audio for processing...', {
                ...
                detected_sounds_count: detectedSounds.length
            });
    ...
    // Add sound detector initialization
    initSoundDetector();
});

// Function to initialize the sound detector
function initSoundDetector() {
    // Only initialize if EmergencySoundDetector is available
    if (typeof EmergencySoundDetector !== 'undefined') {
        console.log('Initializing emergency sound detector...');
        
        // Create and initialize the sound detector
        soundDetector = new EmergencySoundDetector();
        
        soundDetector.initialize().then(success => {
            if (success) {
                console.log('Sound detector initialized successfully');
                
                // Set up detection handler
                soundDetector.onSoundDetected = handleSoundDetection;
                ...
                console.error('Failed to initialize sound detector');
            }
        }).catch(error => {
            console.error('Error initializing sound detector:', error);
        });
    } else {
        console.warn('EmergencySoundDetector not available');
    }
}
...
// Handle sound detection events
function handleSoundDetection(data) {
    console.log('Emergency sound detected:', data);
    
    // Add to the detected sounds array
    detectedSounds.push({
        type: data.type,
        time: data.time,
    ...
    // Limit array size
    if (detectedSounds.length > 10) {
        detectedSounds.shift();
    }
    
    // Optionally display a subtle notification if needed
    // showSoundDetectionNotification(data);
}

// Function to display a subtle notification of detected sound
function showSoundDetectionNotification(data) {
    // Create a notification element
    const notif = document.createElement('div');
    notif.className = 'sound-notification';
    notif.textContent = `Detected: ${data.type}`;
    
    // Add to the conversation log
    conversationLog.appendChild(notif);
